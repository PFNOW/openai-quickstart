...
class OpenAIModel(Model):
    def __init__(self, model: str, api_key: str, base_url: str):
        self.model = model
        self.client = OpenAI(api_key = api_key if api_key else os.getenv("OPENAI_API_KEY"), base_url = base_url if base_url else os.getenv("OPENAI_BASE_URL"))

    def make_request(self, system_prompt, user_prompt):
        attempts = 0
        while attempts < 3:
            try:
                if self.model in ["gpt-3.5-turbo", "gpt-3.5", "gpt-4", "gpt-4o", "gpt-4o-mini", "gpt-o1", "gpt-o1-mini"]:
