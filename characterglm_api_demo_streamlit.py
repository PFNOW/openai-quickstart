...
# 图像提示词
def update_style(key: Optional[str] = None):
    if debug:
        print(f'update_style. st.session_state["IMAGE_STYLE"] = {st.session_state["API_KEY"]}, key = {key}')
    key = key or st.session_state["IMAGE_STYLE"]
    if key:
        return key


image_style = st.sidebar.text_input("图像风格", value="二次元玄幻风格", key="IMAGE_STYLE", type="default", on_change=update_style)
update_style(image_style)
...
with st.container():
    ...
    st.text_area(label="背景设定", key="system_prompt_content",
                 on_change=lambda: st.session_state["meta"].update(system_prompt_content=st.session_state["system_prompt_content"]),
                 help="对话的详细背景设定，不可以为空")
    with col1:
...
def draw_new_image():
    ...
    image_prompt = f'{image_style}。' + image_prompt.strip()
